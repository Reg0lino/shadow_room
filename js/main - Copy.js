...
let resetObjectBtn;
let saveStateBtn, loadStateBtn; // Save/Load buttons
// Environment Sliders
...
function init() {
    try {
        ...
        // Try loading state first. If it fails or doesn't exist, apply defaults.
        ...
        if (!stateLoaded) {
             logToPage("No valid saved state found, applying defaults.");
             // Create initial object if none loaded
             updateObject(shapeSelect.value);
             // Trigger handlers to set initial state based on slider defaults
             onWallColorChange();
             onFloorColorChange();
             // Color already randomized in updateObject
             // onModelColorOrBrightnessChange();
             onObjectMaterialChange();
             onLightIntensityChange();
             onSpotlightParamsChange();
             onLightPositionChange();
             onModelYOffsetChange();
             onObjectRotationChange();
             onObjectScaleChange(); // Apply initial scale

             // Ensure initial UI span values are correct by dispatching events
             // (This is slightly redundant if handlers update spans, but ensures consistency)
             ...
             logToPage("Default control values applied.");
        }
